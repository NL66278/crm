...
class TestMembership(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.partner_club = partner_club = partner_model.create(
            {"name": "The Club", "city": "Rotterdam", "is_company": True}
        )
        ...
        cls.product_club_membership = product_club_membership = product_model.create(
            {
        ...
        cls.product_direct_member = product_direct_member = product_model.create(
            {
                "name": "Test direct_member",
                "type": "service",
                "membership": True,
                "uom_id": cls.uom_unit.id,
                "uom_po_id": cls.uom_unit.id,
        ...
        cls.line_direct_member = line_model.create(
            {
                "analytic_account_id": contract_jan.id,
                "product_id": product_direct_member.id,
                "name": "Personal membership",
                "quantity": 1,
                "uom_id": product_direct_member.uom_id.id,
                "price_unit": 25.0,
                "discount": 50.0,
            }
        )
    ...
    def test_contract(self):
        ...
        self.assertTrue(self.line_direct_member.membership)
        self.assertEqual(
            self.partner_jan.membership_line_ids[0], self.line_direct_member
        )
    ...
    def test_contract_expiration(self):
        ...
        self.assertEqual(self.line_direct_member.date_end, "2017-12-31")
        self.partner_jan._compute_membership()
